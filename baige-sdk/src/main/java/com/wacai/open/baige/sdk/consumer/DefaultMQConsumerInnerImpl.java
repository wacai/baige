    ...
    PullMsgCallback pullMsgCallback = new PullMsgCallback() {
      ...
      public void onSuccess(PullResult pullResult) {
        switch (pullResult.getPullStatus()) {
          ...
          case PULL_NOT_FOUND:
            //通知回调函数消息未拉取到；
           // LOGGER.info("pull msg but not found , topic:{}", pullRequest.getTopic());
            pmc.onMsgNotFound(pullRequest);
            break;
        case TOPIC_NOT_ACK:
            //通知回调函数消息未拉取到；
         //   LOGGER.info("pull msg but not ack , topic:{}", pullRequest.getTopic());
            pmc.onMsgNotAck(pullRequest);
            break;
          default:
            if (pullResult != null && pullResult.getPullStatus() != null) {
              LOGGER.warn("pull msg  get  pull topic:{},status is :{} ", pullRequest.getTopic(),pullResult.getPullStatus());
            }
            break;
        }
      ...
      public void onException(Throwable e) {
        LOGGER.error("use MQClientAPIImpl to pull msg {} catch Exception:{}", pullRequest, e.getMessage());
        pmc.onException(e);
      }
      ...
      public void onMsgNotFound(PullRequest pullRequest) {
        ...
        @Override
        public void onMsgNotAck(PullRequest pullRequest) {
            pmc.onMsgNotAck(pullRequest);
        }
    };
